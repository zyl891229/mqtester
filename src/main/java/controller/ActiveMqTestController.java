...
public class ActiveMqTestController {
    ...
    @ResponseBody
    public void  json(@RequestParam(value = "name") String name,@RequestParam(value = "commit") String commit,@RequestParam(value = "path") String path,HttpServletRequest request,HttpServletResponse response) throws IOException {
    		...
    		System.out.println(name+commit+path+referer);
    		response.sendRedirect(response.encodeRedirectURL("http://www.baidu.com"));  
//    		 return request.getRequestURL().toString()+"      "+name+"      "+commit;
    }
}
